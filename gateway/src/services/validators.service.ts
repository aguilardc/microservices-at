        ...
        return (req: Request, res: Response, next: NextFunction) => {
            ...
            parametersOrigin.forEach((origin: string) => {
                if (schemaJSON.hasOwnProperty(origin)) {
                    switch (origin) {
                        case "body":
                            validationsList.push(schemaJSON[origin].validate(req.body, {abortEarly: false}));
                            break;
                        case "headers":
                            validationsList.push(schemaJSON[origin].validate(req.headers, {abortEarly: false}));
                            break;
                        case "params":
                            validationsList.push(schemaJSON[origin].validate(req.params, {abortEarly: false}));
                            break;
                        case "query":
                            validationsList.push(schemaJSON[origin].validate(req.query, {abortEarly: false}));
                            break;
                    }
                }
            })

            Promise.all(validationsList)
                .then(results => {
